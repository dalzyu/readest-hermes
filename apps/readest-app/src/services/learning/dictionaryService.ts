 ...
 * Throw if 0 entries.
 */
export async function importUserDictionary(input: {
  files: File[];
...
export async function importUserDictionary(
  inputOrZipFile:
    | { files: File[]; sourceLanguage: string; targetLanguage?: string; name?: string }
    | File
    | Uint8Array,
  maybeMeta?: { name: string; language: string; targetLanguage: string },
): Promise<UserDictionary> {
  const input =
    inputOrZipFile instanceof File || inputOrZipFile instanceof Uint8Array
      ? { file: inputOrZipFile, meta: maybeMeta! }
      : {
          file: inputOrZipFile.files[0]!,
          meta: {
            name: inputOrZipFile.name ?? inputOrZipFile.files[0]!.name,
            language: inputOrZipFile.sourceLanguage,
            targetLanguage: inputOrZipFile.targetLanguage ?? inputOrZipFile.sourceLanguage,
          },
        };
  const zipFile = input.file;
...
export async function lookupDictionaryEntries(input: {
  term: string;
  sourceLanguage: string;
  targetLanguage?: string;
  fuzzy?: boolean;
  ...
  return lookupDefinitions(
    ...
    input.targetLanguage ?? input.sourceLanguage,
    [],
    {
      maxMatchTier: input.fuzzy === false ? 1 : 4,
    },
  );
}
