...
export async function markVocabularyEntryReviewed(
  entryOrId: VocabularyEntry | string,
  quality: 0 | 1 | 2 | 3 | 4 | 5 | 'again' | 'good' | 'easy' = 3,
  now = Date.now(),
): Promise<VocabularyEntry> {
  const entry =
    typeof entryOrId === 'string'
      ? (await getAllVocabulary()).find((item) => item.id === entryOrId)
      : entryOrId;
...
export async function getDueVocabularyForBook(
  bookHash: string,
  now = Date.now(),
): Promise<VocabularyEntry[]> {
