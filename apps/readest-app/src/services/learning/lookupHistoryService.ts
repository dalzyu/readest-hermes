import type { TranslationResult } from '@/services/learning/types';
...
export function saveLookupHistoryEntry(raw: LookupHistoryInput): LookupHistoryEntry | undefined {
...
export function clearLookupHistoryForBook(bookHash: string): void {
  if (typeof localStorage === 'undefined') return;
  const normalizedBookHash = bookHash.trim();
  if (!normalizedBookHash) return;
  saveHistory(loadHistory().filter((entry) => entry.bookHash !== normalizedBookHash));
}
